...
func creatorFullView(cr model.Creator) gin.H {
	...
	return gin.H{
		...
		"verify_reject_fields": splitRejectFields(cr.VerifyRejectFields),
		// 核验方式/时间透出：审核员据此分清"四要素真过(biz_4e/bankcard3)"还是"降级未核验(manual)"，不再闭眼通过。
		"verify_method":       cr.VerifyMethod,
		"verify_checked_at":   cr.VerifyCheckedAt,
